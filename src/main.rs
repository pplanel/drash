...
use clap::Parser;
use drash::{ahash, dhash, hamming_distance, load_resize_grayscale, phash, distance_str, DrashError};
...
fn main() -> Result<(), DrashError> {
    ...
    let hash1 = match opts.algorithm.as_str() {
        ...
        _ => Err(DrashError::UnsupportedAlgorithm(opts.algorithm.clone())),
    }?;
    ...
    let size = match opts.algorithm.as_str() {
        ...
        "ahash" => 64,
        "phash" => 64,
        _ => return Err(DrashError::UnsupportedAlgorithm(opts.algorithm.clone())),
    };
