...
impl fmt::Display for DrashError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            DrashError::ImageError(e) => write!(f, "Image error: {}", e),
            DrashError::UnsupportedAlgorithm(algo) => write!(f, "Unsupported algorithm: {}", algo),
        }
    }
}

impl From<ImageError> for DrashError {
    fn from(err: ImageError) -> DrashError {
        DrashError::ImageError(err)
    }
}

impl Error for DrashError {}

pub fn load_resize_grayscale(image_path: &Path) -> Result<ImageBuffer<Luma<u8>, Vec<u8>>, DrashError> {
...
pub fn ahash(image_path: &Path) -> Result<u64, DrashError> {
...
pub fn phash(image_path: &Path) -> Result<u64, DrashError> {
    let image = open(image_path)?.to_luma8();
